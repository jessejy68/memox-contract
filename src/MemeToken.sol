...
contract MemeToken is ERC20Burnable {
     ...
     */
    modifier onlyMeme() {
        if (msg.sender != metaNodeCore) revert onlyMemeCall();
        _;
    }
     ...
     * @param name 代币名称
     * @param symbol 代币符号
     * @param totalSupply 总供应量（含 18 位小数）
    ...
    /**
     * @notice 更新余额钩子 - 实现转账限制逻辑
     * @dev 重写 ERC20 的 _update
     *
     * @param from 发送方地址
     * @param to 接收方地址
     * @param amount 转账金额
