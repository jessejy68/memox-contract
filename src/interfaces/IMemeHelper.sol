...
interface IMemeHelper {
     ...
     */
    function calculateTokenAmountOut(uint256 ethIn, IMemeCore.BondingCurveParams memory curve) external pure returns (uint256);
     ...
     */
    function calculateRequiredETH(uint256 tokenOut, IMemeCore.BondingCurveParams memory curve) external pure returns (uint256);
     ...
     * @return feeETH 手续费
     */
    function calculateTokenAmountOutWithFee(uint256 ethIn, IMemeCore.BondingCurveParams memory curve, uint256 feeRate) external pure returns (uint256 tokenOut, uint256 netETH, uint256 feeETH);
     ...
     */
    function calculateETHAmountOut(uint256 tokenIn, IMemeCore.BondingCurveParams memory curve) external pure returns (uint256);
     ...
     * @return feeETH 手续费
     */
    function calculateETHAmountOutWithFee(uint256 tokenIn, IMemeCore.BondingCurveParams memory curve, uint256 feeRate) external pure returns (uint256 netETH, uint256 feeETH);
     ...
     * 4. LP 代币发送到死地址（永久锁定）
     */
    function addLiquidityV2(address token, uint256 ethAmount, uint256 tokenAmount) external payable returns (uint256 liquidity);

    /**
     * @notice 获取当前代币价格
     * @param curve 曲线参数
     * @return 价格（ETH/Token × 1e18）
     * 
