 ...
 * 
 * 【买入流程】
 * 用户支付 ETH → 扣除手续费 → 按曲线计算代币量 → 转移代币
 * 
 * 【卖出流程】
 * 用户转入代币 → 按曲线计算 ETH → 扣除手续费 → 转移 ETH
 * 
 * 【毕业流程】
...
interface IMemeCore is IVestingParams, IBondingCurveParams {
     ...
     */
    struct TokenInfo {
        /// @notice 创建者地址（接收毕业创作者奖励）
        address creator;
        /// @notice 创建时间戳
        uint256 createdAt;
        /// @notice 开盘时间（0 表示立即开盘）
        uint256 launchTime;
        /// @notice 当前状态
        TokenStatus status;
        /// @notice 预计算的 DEX 交易对地址
    ...
    event TokenGraduated(
        address indexed token,      // 代币地址
        uint256 liquidityETH,       // 添加的流动性 ETH
    ...
    event TokenCreatedWithInitialBuy(
        ...
        uint256 initialETHSpent,         // 初始购买花费的 ETH
     ...
     * @param deadline 交易截止时间
     */
    function sell(address token, uint256 tokenAmount, uint256 minETHAmount, uint256 deadline) external;
