...
import {IMemeCore as IMemeCore} from "./interfaces/IMemeCore.sol";
import {IMemeFactory as IMemeFactory} from "./interfaces/IMemeFactory.sol";
import {IMemeHelper as IMemeHelper} from "./interfaces/IMemeHelper.sol";
import {IMemeVesting as IMemeVesting} from "./interfaces/IMemeVesting.sol";
 ...
 * 
 * 【买入代币】
 * 用户支付 ETH → 扣除手续费 → 按曲线计算代币量
 *      ↓
 * 更新曲线储备 → 转移代币 → 检查是否触发毕业
 * 
 * 【卖出代币】
 * 用户转入代币 → 按曲线计算 ETH → 扣除手续费
 *      ↓
 * 更新曲线储备 → 转移 ETH
 * 
 * 【毕业代币】
 ...
 */
contract MemeCore is IMemeCore, Initializable, UUPSUpgradeable, AccessControlUpgradeable, PausableUpgradeable, ReentrancyGuard {
    ...
    function initialize(
        ...
        // 设置默认费率
        creationFee = 0.05 ether;          // 创建费：0.05 ETH
    ...
    function createToken(
        ...
        uint256 adjustedETHReserve = params.virtualETHReserve;
        uint256 adjustedTokenReserve = params.virtualTokenReserve;
        uint256 preBuyFee;

        // 添加保证金
        if (params.marginEth > 0) {
            totalPaymentRequired += params.marginEth;
        }

        // 计算初始买入
        if (params.initialBuyPercentage > 0) {
            (initialTokens, initialETH, adjustedETHReserve, adjustedTokenReserve) =
            ...
            preBuyFee = (initialETH * preBuyFeeRate) / 10000;
        ...
        bondingCurve[tokenAddress] = BondingCurveParams({
            virtualETHReserve: adjustedETHReserve,
            virtualTokenReserve: adjustedTokenReserve,
            k: params.virtualETHReserve * params.virtualTokenReserve,
            availableTokens: params.saleAmount - initialTokens,
            collectedETH: initialETH
        });
        ...
        // 9. 设置转账模式
        MemeToken(tokenAddress).setTransferMode(
            MemeToken.TransferMode.MODE_TRANSFER_CONTROLLED
        );
        ...
        if (params.marginEth > 0) {
            ...
            payable(marginReceiver).transfer(params.marginEth);
     ...
     *    - netETHAmount = msg.value - tradingFee
     * 
     * 4. 【代币计算】
    ...
    function buy(
        ...
        uint256 netETHAmount = msg.value - tradingFee;
        
        // 4. 代币计算
        uint256 tokenAmount = helper.calculateTokenAmountOut(netETHAmount, bondingCurve[token]);

        // 5. 余量处理
        if (tokenAmount > bondingCurve[token].availableTokens) {
            tokenAmount = bondingCurve[token].availableTokens;
            netETHAmount = helper.calculateRequiredETH(tokenAmount, bondingCurve[token]);
            tradingFee = (netETHAmount * tradingFeeRate) / (10000 - tradingFeeRate);
            uint256 actualPayment = netETHAmount + tradingFee;
     ...
     * @param minETHAmount 最小获得 ETH（滑点保护）
     * @param deadline 交易截止时间
     * 
     * ============ 执行流程 ============
     * 
     ...
     * 
     * 2. 【ETH 计算】
     *    - 使用 helper 按曲线计算可获得 ETH
     *    - 计算手续费
     *    - netETHAmount = ethAmount - tradingFee
     * 
     * 3. 【滑点校验】
     *    - netETHAmount >= minETHAmount
     * 
     * 4. 【池余额校验】
     *    - ethAmount <= collectedETH
     * 
     * 5. 【转入代币】
     *    - 从用户转入代币到合约
     * 
    ...
    function sell(
        ...
        // 2. ETH 计算
        uint256 ethAmount = helper.calculateETHAmountOut(tokenAmount, bondingCurve[token]);
        uint256 tradingFee = (ethAmount * tradingFeeRate) / 10000;
        uint256 netETHAmount = ethAmount - tradingFee;

        // 3-4. 校验
        if (netETHAmount < minETHAmount) revert SlippageExceeded();
        if (ethAmount > bondingCurve[token].collectedETH) revert InsufficientBalance();

        // 5. 转入代币
        IERC20(token).safeTransferFrom(msg.sender, address(this), tokenAmount);

        // 6. 更新曲线
        bondingCurve[token].virtualETHReserve -= ethAmount;
        bondingCurve[token].virtualTokenReserve += tokenAmount;
        bondingCurve[token].availableTokens += tokenAmount;
        bondingCurve[token].collectedETH -= ethAmount;

        // 7. 转移 ETH
        _sendValue(platformFeeReceiver, tradingFee);
        _sendValue(msg.sender, netETHAmount);
     ...
     * 
     * 1. 【读取数据】
     *    - 获取 collectedETH 和 availableTokens
     * 
     * 2. 【计算费用分配】
     *    - platformFee = collectedETH × graduationPlatformFeeRate / 10000
     *    - creatorFee = collectedETH × graduationCreatorFeeRate / 10000
     *    - liquidityETH = collectedETH - platformFee - creatorFee
     *    - 代币同样按比例分配
     * 
     * 3. 【解除转账限制】
     *    - 切换代币为 NORMAL 模式
    ...
    function graduateToken(address token) external onlyRole(DEPLOYER_ROLE) validToken(token) nonReentrant {
        ...
        // 1. 读取数据
        uint256 collectedETH = curve.collectedETH;
        uint256 remainingTokens = curve.availableTokens;

        // 2. 计算费用分配
        uint256 platformFee = collectedETH * graduationPlatformFeeRate / 10000;
        uint256 creatorFee = collectedETH * graduationCreatorFeeRate / 10000;
        uint256 liquidityETH = collectedETH - platformFee - creatorFee;
        ...
        uint256 liquidityResult = helper.addLiquidityV2{value: liquidityETH}(token, liquidityETH, liquidityTokens);
        ...
        emit TokenGraduated(token, liquidityETH, liquidityTokens, liquidityResult);
    }
     ...
     */
    function calculateBuyAmount(address token, uint256 ethAmount) external view returns (uint256 tokenAmount) {
        BondingCurveParams memory curve = bondingCurve[token];
        tokenAmount = helper.calculateTokenAmountOut(ethAmount, curve);
     ...
     */
    function calculateBuyAmountWithFee(address token, uint256 ethAmount) external view returns (uint256 tokenOut, uint256 netETH, uint256 feeETH) {
        BondingCurveParams memory curve = bondingCurve[token];
        (tokenOut, netETH, feeETH) = helper.calculateTokenAmountOutWithFee(ethAmount, curve, tradingFeeRate);
        if (tokenOut > curve.availableTokens) {
            tokenOut = curve.availableTokens;
            netETH = helper.calculateRequiredETH(tokenOut, curve);
            feeETH = (netETH * tradingFeeRate) / (10000 - tradingFeeRate);
        }
    }

    /**
     * @notice 计算卖出可获得 ETH（不含手续费）
     */
    function calculateSellReturn(address token, uint256 tokenAmount) external view returns (uint256) {
        return helper.calculateETHAmountOut(tokenAmount, bondingCurve[token]);
    }

    /**
     * @notice 计算卖出可获得 ETH（含手续费明细）
     */
    function calculateSellReturnWithFee(address token, uint256 tokenAmount) external view returns (uint256 netETH, uint256 feeETH) {
        (netETH, feeETH) = helper.calculateETHAmountOutWithFee(tokenAmount, bondingCurve[token], tradingFeeRate);
    }

    /**
     * @notice 计算初始买入所需 ETH
     * @param totalSupply 总供应量
     * @param virtualETHReserve 初始虚拟 ETH 储备
     * @param virtualTokenReserve 初始虚拟代币储备
     * @param percentageBP 购买百分比（基点）
     * @return totalPayment 总支付金额（含手续费）
     * @return preBuyFee 初始买入手续费
     */
    function calculateInitialBuyETH(
        uint256 totalSupply,
        ...
        (, uint256 ethRequired,,) = _calculateInitialBuy(
            totalSupply,
            virtualETHReserve,
            virtualTokenReserve,
            percentageBP
        );
        preBuyFee = (ethRequired * preBuyFeeRate) / 10000;
        totalPayment = ethRequired + preBuyFee;
    }
    ...
    function _sendValue(address to, uint256 amount) internal {
        ...
        if (size > 0) {
            ...
            if (!ok) {
                ...
                require(fallbackSuccess, "ETH_SEND_FAILED_TO_FALLBACK");
    ...
    function _createVestingSchedules(
        ...
        if (tokensToBurn > 0) {
            ...
            emit MemeTokensBurned(tokenAddress, tokensToBurn);
        }
    ...
    function _calculateInitialBuy(
        ...
        // 注意：这个检查是防御性的，因为调用者（createToken 和 calculateInitialBuyETH）已经检查过了
        // 但如果 percentageBP 为 0，这里不会检查，因为 0 是合法的（表示不进行初始买入）
        if (percentageBP > 0 && percentageBP > MAX_INITIAL_BUY_PERCENTAGE) revert InvalidPercentageBP();
        ...
        ethRequired = newETHReserve - virtualETHReserve;

        return (tokensOut, ethRequired, newETHReserve, newTokenReserve);
    }
