 ...
 * @title MEMEFactory
 * @author Meme Team
 * @notice 代币工厂合约 - 负责部署新的 MemeToken
 * 
 * ============ 合约职责 ============
...
contract MemeFactory is IMemeFactory, AccessControl {
    ...
    /**
     * @notice 部署者角色标识
     * @dev 拥有此角色才能调用 deployToken
     * 仅 MEMECore 合约应被授予此角色
     */
    bytes32 public constant DEPLOYER_ROLE = keccak256("DEPLOYER_ROLE");

    /**
     * @notice Meme 核心合约地址
     * @dev 部署的代币会将全部供应量铸造给此地址
     * 通过 setMeme 设置，同时授予 DEPLOYER_ROLE
     */
    address public metaNode;

    // ============ 构造函数 ============

    /**
     * @notice 初始化工厂合约
     * @param _admin 管理员地址，拥有 DEFAULT_ADMIN_ROLE
     * 
     * 初始状态：
     * - metaNode = address(0)（需后续设置）
     * - 无人拥有 DEPLOYER_ROLE
     */
     ...
     * 2. 使用 CREATE2 部署 MemeToken
     * 3. MemeToken 构造函数将所有代币铸造给 metaNode
    ...
    function deployToken(
        ...
        // 计算 CREATE2 盐值
        bytes32 salt = keccak256(abi.encodePacked(name, symbol, totalSupply, metaNode, timestamp, nonce));
        
        // 部署代币合约，所有代币铸造给 metaNode
        MemeToken token = new MemeToken{salt: salt}(name, symbol, totalSupply, metaNode);
     ...
     * - 建议在初始化时调用一次
     */
    function setMeme(address _metaNode) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(_metaNode != address(0), "ZeroAddress");
