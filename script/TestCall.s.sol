 ...
 * 部署命令示例：
 * Base Sepolia 测试网：
 * forge script script/TestCall.s.sol:TestCall --rpc-url $BASE_SEPOLIA_RPC --broadcast --verify --etherscan-api-key $BASESCAN_API_KEY --verifier basescan --legacy
 *
 * Base 主网：
 * forge script script/TestCall.s.sol:TestCall --rpc-url $BASE_MAIN_RPC --broadcast --verify --etherscan-api-key $BASESCAN_API_KEY --verifier basescan --legacy
 *

 * forge script script/TestCall.s.sol:TestCall --rpc-url $SEPOLIA_MAIN_RPC --broadcast --verify --etherscan-api-key $ETH_API_KEY --verifier etherscan --legacy
 */
...
contract TestCall is Deployer {
    ...
    function setUp() public override {
        // ===== 选择测试网络（取消注释对应行）=====
//        projectName = "sepolia/";           // Sepolia 主网
//        projectName = "bnb_test/";      // Sepolia 测试网
...
//        environment = "prod";           // 生产环境
    ...
    function run() public {
        ...
        // 初始化合约实例
        core = MemeCore(payable(cfg.MemeCore()));
...
//        buyTokens(tokenAddress, ethAmount);                              // 联合曲线买入
//        addLiquidityToPancakeSwapV2(tokenAddress, tokenAmount, ethAmount); // 添加流动性
//        buyTokenOnPancake(tokenAddress, ethAmount);                       // DEX 买入
...
//        getLPTokenBalance(tokenAddress);   // 查询 LP 代币余额
    ...
    function getTokenInfo(address tokenAddress) public view {
        ...
        console.log("  collectedETH:", bondingCurve.collectedETH);
        console.log("  k:", bondingCurve.k);
        console.log("  virtualTokenReserve:", bondingCurve.virtualTokenReserve);
        console.log("  virtualETHReserve:", bondingCurve.virtualETHReserve);
    }
    ...
    function createToken() public returns (address tokenAddress){
        ...
        params.virtualETHReserve = 8219178082191780000;  // 虚拟 ETH 储备
        ...
        params.marginEth = 1 ether;               // 保证金 1 BNB
     ...
     */
    function buyTokens(address tokenAddress, uint256 ethAmount) public {
        // 预估可获得的代币数量
        uint256 estimatedTokens = core.calculateBuyAmount(tokenAddress, ethAmount);
     ...
     */
    function addLiquidityToPancakeSwapV2(address tokenAddress, uint256 tokenAmount, uint256 ethAmount) public {
        uint256 deployerPrivateKey = vm.envUint("PRIVATE_KEY");
        ...
        uint256 minBNBAmount = ethAmount * 95 / 100;
        uint256 deadline = block.timestamp + 300;
    ...
    function removeLiquidityFromPancake(address tokenAddress) public {
        ...
        // 获取交易对地址
        address pairAddress = IPancakeFactory(router.factory()).getPair(tokenAddress, weth);
     ...
     * @param ethAmount 支付的 ETH 数量
     */
    function buyTokenOnPancake(address tokenAddress, uint256 ethAmount) public {
        uint256 deployerPrivateKey = vm.envUint("PRIVATE_KEY");
    ...
    function sellTokenOnPancake(address tokenAddress, uint256 tokenAmount) public {
        ...
        console.log("Expected ETH to receive:", expectedBNBAmount / 1e18, "BNB");

        // 设置滑点保护（1%）
        uint256 minBNBAmount = expectedBNBAmount * 99 / 100;
        uint256 deadline = block.timestamp + 300;

        uint256 initialETHBalance = seller.balance;
        ...
        uint256 ethReceived = finalBNBBalance - initialETHBalance;

        console.log("BNB received:", ethReceived / 1e18, "BNB");
    }
    ...
    function getTokenPriceInfo(address tokenAddress) public view {
        ...
        address pairAddress = IPancakeFactory(factoryAddr).getPair(tokenAddress, weth);
        ...
        // 计算价格：1 代币 = ? BNB
        uint256 priceInETH = (wethReserve * 1e18) / tokenReserve;

        console.log("Token address:", tokenAddress);
        console.log("Token reserve:", tokenReserve / 1e18);
        console.log("WETH reserve:", wethReserve / 1e18);
        console.log("Price (1 Token):", priceInETH, "wei BNB");
        console.log("Price (1 Token):", priceInETH / 1e14, "e-4 BNB");
    }
