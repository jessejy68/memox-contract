...
help:
	...
	@echo ""
	@echo "部署命令（需要配置环境变量）："
	@echo "  make deploy-base-test         - 部署到 Base Sepolia 测试网"
	@echo "  make deploy-base-test-no-verify - 部署到 Base Sepolia 测试网（不验证）"
	@echo "  make deploy-base              - 部署到 Base 主网"
	@echo "  make verify                   - 验证合约"
	@echo ""
	@echo "环境变量配置："
	@echo "  PRIVATE_KEY         - 部署者私钥"
	@echo "  BASE_SEPOLIA_RPC       - Base Sepolia 测试网 RPC"
	@echo "  BASE_MAIN_RPC       - Base 主网 RPC"
...
test:
	forge test --no-match-contract "BaseTest|BaseTest"

# 运行所有本地测试（详细输出）
test-v:
	forge test --no-match-contract "BaseTest|BaseTest" -vvv

# 运行所有本地测试（显示测试摘要表格）
test-summary:
	forge test --no-match-contract "BaseTest|BaseTest" --summary

# 运行测试并显示 gas 报告
test-gas:
	forge test --no-match-contract "BaseTest|BaseTest" --gas-report
...
# Sepolia 测试网 fork 测试
# 需要设置环境变量：export BASE_SEPOLIA_RPC=https://data-seed-prebsc-1-s1.binance.org:8545
test-fork-base:
	forge test --match-contract BaseTest -vvv

# Base 测试网 fork 测试
# 需要设置环境变量：export BASE_SEPOLIA_RPC=<your_base_rpc>
test-fork-base:
	forge test --match-contract BaseTest -vvv
...
# 生成测试覆盖率报告
coverage:
	forge coverage --no-match-contract "BaseTest|BaseTest"

# 生成 lcov 格式覆盖率报告
coverage-lcov:
	forge coverage --no-match-contract "BaseTest|BaseTest" --report lcov

# ============ 部署命令 ============
# 注意：部署前需要配置以下环境变量：
#   PRIVATE_KEY         - 部署者私钥
#   BASE_SEPOLIA_RPC       - Base Sepolia 测试网 RPC（如：https://sepolia.base.org）
#   BASE_MAIN_RPC       - Base 主网 RPC（如：https://mainnet.base.org）
#   BASESCAN_API_KEY    - BaseScan API Key（用于合约验证）

# 部署到 Base Sepolia 测试网
deploy-base-test:
	@echo "部署到 Base Sepolia 测试网..."
	@echo "请确保已设置 PRIVATE_KEY 和 BASE_SEPOLIA_RPC 环境变量"
...
deploy-base-test-no-verify:
	@echo "部署到 Base Sepolia 测试网（不验证）..."
	forge script script/Deploy.s.sol:DeployScript \
		--rpc-url $(BASE_SEPOLIA_RPC) \
		--broadcast \
		--legacy \
...
verify:
	@echo "验证合约..."
	forge verify-contract $(CONTRACT_ADDRESS) MemeCore \
		--chain-id 84532 \
		--etherscan-api-key $(BASESCAN_API_KEY) \
		--watch
...
list-tests:
	...
	@echo "  - BaseTest             : BSC Fork测试 (需要RPC)"
	@echo "  - BaseTest          : Base Fork测试 (需要RPC)"
...
.PHONY: build clean test test-v test-summary test-gas \
        test-core test-vesting test-fee test-margin \
        test-future-launch test-calculate test-vesting-prebuy test-vanity \
        test-fork-base test-fuzz \
        coverage coverage-lcov list-tests fmt fmt-check \
        deploy-base-test deploy-base-test-no-verify deploy-base deploy-core-impl deploy-factory \
