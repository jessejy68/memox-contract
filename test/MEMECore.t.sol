...
contract MemeCoreTest is Test {
    ...
    function _signParams(IMemeCore.CreateTokenParams memory params) internal view returns (bytes memory) {
    ...
    function _basicParams() internal view returns (IMemeCore.CreateTokenParams memory p) {
        p = IMemeCore.CreateTokenParams({
            name: "Extra",
            symbol: "EXT",
    ...
    function testOnlyPauserCanPauseToken() public {
        IMemeCore.CreateTokenParams memory p = _basicParams();
        bytes memory sig = _signParams(p);
        (uint256 pay,uint256 preBuyFee) = core.calculateInitialBuyETH(p.totalSupply, p.virtualETHReserve, p.virtualTokenReserve, p.initialBuyPercentage);
    ...
    function testCreateTokenWithMargin() public {
        ...
        vm.prank(creator);
        core.createToken{value: pay}(abi.encode(p), sig);
        assertEq(platform.balance, balBefore + core.creationFee() + p.marginEth + preBuyFee);
    }
    ...
    function testBuyExceedAvailableShouldRevert() public {
        ...
        vm.expectRevert(IMemeCore.SlippageExceeded.selector);
        vm.prank(user);
        core.buy{value: 100 ether}(token, curve.availableTokens * 2, block.timestamp + 100);
    }

    /**
     * @notice 测试卖出超过池中已收集 ETH 应该失败
     * @dev 当联合曲线中没有收集到足够的 ETH 时，卖出应该 revert
     */
    function testSellExceedCollectedETHShouldRevert() public {
        IMemeCore.CreateTokenParams memory p = _basicParams();
        p.initialBuyPercentage = 0;
        bytes memory sig = _signParams(p);
        vm.prank(creator);
        ...
        vm.expectRevert(IMemeCore.InsufficientBalance.selector);
        vm.prank(user);
    ...
    function testCannotTradeIfBlacklisted() public {
        ...
        vm.expectRevert(IMemeCore.TokenNotTrading.selector);
        vm.prank(user);
