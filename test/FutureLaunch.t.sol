...
contract FutureLaunchTest is Test {
    ...
    function testFutureLaunchPreventsEarlyTrading() public {
        ...
        IMemeCore.CreateTokenParams memory params = IMemeCore.CreateTokenParams({
            ...
            virtualETHReserve: 1 ether,           // 虚拟 ETH 储备：1 ETH
            ...
            marginEth: 0,
            marginTime: 0,
            vestingAllocations: new IMemeCore.VestingAllocation[](0) // 无锁仓分配
        });
        ...
        // 验证代币信息
        IMemeCore.TokenInfo memory info = core.getTokenInfo(tokenAddress);
        assertEq(uint8(info.status), uint8(IMemeCore.TokenStatus.TRADING), "Status should be TRADING");
        assertEq(info.launchTime, futureTime, "Launch time should be set correctly");
        
        // 尝试在启动时间之前买入 - 应该失败
        vm.prank(buyer);
        vm.expectRevert(IMemeCore.TokenNotLaunchedYet.selector);
