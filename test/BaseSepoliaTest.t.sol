...
contract BaseSepoliaTest is Test {
    address constant PANCAKE_V2_ROUTER = 0xB032ab6808C8b4af5FA397E1181eAFD9ac1fd870;
    ...
    uint256 constant VIRTUAL_ETH_RESERVE = 8219178082191780000;
    ...
    function setUp() public {
        ...
        helper = new MemeHelper(admin, PANCAKE_V2_ROUTER, WETH);
    ...
    function _createTokenHelper() internal returns (address token) {
        ...
        params.virtualETHReserve = VIRTUAL_ETH_RESERVE;
        ...
        uint256 preBuyFeeAmount = (initialETH * preBuyFeeRate) / 10000;
        uint256 totalPayment = creationFee + initialETH + preBuyFeeAmount;
    ...
    function testTrueCall() public {
        ...
        core = MemeCore(payable(address(0x51605F8D522f18A048F3C1C6d651B5238962acA6)));
...
//        IMemeCore.TokenInfo memory info = core.getTokenInfo(createdToken);
//        console.log(uint256(info.status));
    ...
    function testGraduateToken() public {
        ...
        assert(uint(info.status) == uint(IMemeCore.TokenStatus.GRADUATED));
    }

    function _calculateBNBNeededForTokens(address token, uint256 tokenAmount) internal view returns (uint256) {
        IMemeCore.BondingCurveParams memory curve = core.getBondingCurve(token);
        ...
        return newETHReserve - curve.virtualETHReserve;
    }
    ...
    function _createTokenWithFixedParams(
        ...
        uint256 totalPayment = creationFee + initialETH;
    ...
    function testBuyExpiredDeadline() public {
        vm.expectRevert(IMemeCore.TransactionExpired.selector);
        vm.prank(user);
    ...
    function testCalculateInitialBuyETHExceedMaxShouldFail() public {
        vm.expectRevert(IMemeCore.InvalidParameters.selector);
        core.calculateInitialBuyETH(100, 1 ether, 100 ether, 10_000);
    }
    ...
    function testCalculateFunctions() public view {
        ...
        uint256 eth = helper.calculateTokenAmountOut(1000 ether, curve);
        assertGt(tokens, 0);
        assertGt(eth, 0);
    }
}
