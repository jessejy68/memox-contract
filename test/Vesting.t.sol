...
contract VestingTest is Test {
    ...
    function testCreateTokenWithVesting() public {
        ...
        uint256 totalPayment = core.creationFee() + initialETH;
    ...
    function testMultiVestingSchedules() public {
        ...
        IMemeCore.CreateTokenParams memory params = IMemeCore.CreateTokenParams({
            ...
            virtualETHReserve: 300000000000000000,
    ...
    function testPartialVesting() public {
        ...
        assertApproxEqAbs(MemeToken(tokenAddress).balanceOf(creator), immediateTokens, 1 ether, "Creator should have 30% tokens immediately");

        // Check vesting contract has the other 50%
        uint256 vestingBalance = MemeToken(tokenAddress).balanceOf(address(vesting));
    ...
    function testVestingWithNoInitialBuy() public {
        ...
        uint256 creatorBalance = MemeToken(tokenAddress).balanceOf(creator);
    ...
    function testInvalidVestingAllocations() public {
        ...
        vm.prank(creator);
        vm.expectRevert(IMemeCore.InvalidDurationParameters.selector);
        core.createToken{value: totalPayment}(data, signature);
    }
    ...
    function testFeeCalculationAtGraduation() public {
        ...
        IMemeCore.BondingCurveParams memory curve = core.getBondingCurve(tokenAddress);
        uint256 totalCollectedETH = curve.collectedETH;

        uint256 expectedPlatformETH = (totalCollectedETH * 550) / 10000; // 5.5%
        uint256 expectedCreatorETH = (totalCollectedETH * 250) / 10000;  // 2.5%

        uint256 platformETHReceived = platformFeeReceiver.balance - platformBalanceBefore;
        uint256 creatorETHReceived = creator.balance - creatorBalanceBefore;

        assertApproxEqAbs(platformETHReceived, expectedPlatformETH, 1e15, "Platform should receive 5.5% ETH");
        assertApproxEqAbs(creatorETHReceived, expectedCreatorETH, 1e15, "Creator should receive 2.5% ETH");
    }

    function testRecommendedPoolRatios() public {
        uint256[] memory virtualETHReserves = new uint256[](4);
        virtualETHReserves[0] = 0.3 ether;
        virtualETHReserves[1] = 2.02 ether;
        virtualETHReserves[2] = 3.03 ether;
        virtualETHReserves[3] = 6.06 ether;


        for (uint256 i = 0; i < virtualETHReserves.length; i++) {
            IMemeCore.CreateTokenParams memory params = IMemeCore.CreateTokenParams({
                ...
                virtualETHReserve: virtualETHReserves[i],
            ...
            IMemeCore.BondingCurveParams memory curve = core.getBondingCurve(tokenAddress);
            uint256 calculatedPrice = (curve.virtualETHReserve * 1e18) / curve.virtualTokenReserve;

            // For standard pool: 0.3 ETH / 800M tokens = 0.375e-9 ETH per token
    ...
    function testPoolRatioCreation(string memory name, uint256 virtualETHReserve, bool shouldSucceed, string memory message) internal {
        IMemeCore.CreateTokenParams memory params = IMemeCore.CreateTokenParams({
            name: name,
            symbol: "BOUND",
            totalSupply: 1000000 ether,
            ...
            creator: creator,
            timestamp: block.timestamp,
            requestId: keccak256(abi.encodePacked("boundary-test-", virtualETHReserve)),
            nonce: 400,
            initialBuyPercentage: 0,
            marginEth: 0,
            marginTime: 0,
    ...
    function testDynamicFeeAdjustment() public {
        ...
        uint256 totalPayment = newCreationFee + initialETH;
